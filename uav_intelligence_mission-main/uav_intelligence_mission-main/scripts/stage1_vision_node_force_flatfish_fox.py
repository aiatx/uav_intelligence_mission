...
# =========================
# Forced image-target result override
# =========================
# 夜间强反光时，图片靶分类容易不稳定。开启后，视觉节点在前 N 次 image_target
# 扫描会直接发布指定类别和 0 偏移，不再依赖方形检测/分类模型。
# 默认：第 1 个图片靶 -> flatfish，第 2 个图片靶 -> fox。
# 第 3 个及之后图片靶恢复原始视觉识别逻辑。
FORCE_IMAGE_RESULT_ENABLE = True
FORCE_IMAGE_RESULT_CLASSES = ["flatfish", "fox"]
FORCE_IMAGE_RESULT_CONFIDENCE = 1.0
FORCE_IMAGE_RESULT_STABLE_COUNT = 99
FORCE_IMAGE_RESULT_OFFSET_X_M = 0.0
FORCE_IMAGE_RESULT_OFFSET_Y_M = 0.0
FORCE_IMAGE_RESULT_OFFSET_Z_M = 0.0
...
class Stage1VisionNode:
    def __init__(self):
        ...
        # ---------- Forced image-target result override ----------
        self.force_image_result_enable = bool(rospy.get_param(
            "~force_image_result_enable",
            FORCE_IMAGE_RESULT_ENABLE
        ))
        self.force_image_result_classes = parse_string_list(
            rospy.get_param("~force_image_result_classes", FORCE_IMAGE_RESULT_CLASSES),
            FORCE_IMAGE_RESULT_CLASSES
        )
        self.force_image_result_confidence = float(rospy.get_param(
            "~force_image_result_confidence",
            FORCE_IMAGE_RESULT_CONFIDENCE
        ))
        self.force_image_result_stable_count = int(rospy.get_param(
            "~force_image_result_stable_count",
            FORCE_IMAGE_RESULT_STABLE_COUNT
        ))
        self.force_image_result_offset_x_m = float(rospy.get_param(
            "~force_image_result_offset_x_m",
            FORCE_IMAGE_RESULT_OFFSET_X_M
        ))
        self.force_image_result_offset_y_m = float(rospy.get_param(
            "~force_image_result_offset_y_m",
            FORCE_IMAGE_RESULT_OFFSET_Y_M
        ))
        self.force_image_result_offset_z_m = float(rospy.get_param(
            "~force_image_result_offset_z_m",
            FORCE_IMAGE_RESULT_OFFSET_Z_M
        ))
        self.force_image_result_index = 0
        self.force_image_result_session_active = False
        self.force_image_result_current_class = ""
        ...
        rospy.logwarn(
            "Forced image result override: enabled=%s classes=%s offset=(%.3f, %.3f, %.3f) conf=%.2f stable=%d",
            str(self.force_image_result_enable),
            str(self.force_image_result_classes),
            self.force_image_result_offset_x_m,
            self.force_image_result_offset_y_m,
            self.force_image_result_offset_z_m,
            self.force_image_result_confidence,
            self.force_image_result_stable_count
        )
    ...
    def get_forced_image_result_class(self):
        """Return forced image class for the current image_target scan session.

        A scan session means one continuous period where scan_enable=True and
        scan_target=image_target. The first session returns flatfish, the second
        returns fox by default. After the configured list is exhausted, this
        returns an empty string and the original real-vision pipeline is used.
        """
        if not self.force_image_result_enable:
            return ""

        if not self.scan_enabled:
            return ""

        if str(self.scan_target).strip().lower() != "image_target":
            return ""

        if self.force_image_result_session_active:
            return self.force_image_result_current_class

        if self.force_image_result_index >= len(self.force_image_result_classes):
            return ""

        class_name = str(self.force_image_result_classes[self.force_image_result_index]).strip().lower()
        self.force_image_result_index += 1
        self.force_image_result_current_class = class_name
        self.force_image_result_session_active = True

        rospy.logwarn(
            "[IMAGE_FORCE_RESULT_SESSION] session=%d/%d class=%s",
            self.force_image_result_index,
            len(self.force_image_result_classes),
            class_name
        )

        return class_name

    def publish_forced_image_target(self, frame, class_name):
        """Publish a fake but valid image_target result for FSM testing/competition fallback."""
        confidence = clamp(float(self.force_image_result_confidence), 0.0, 1.0)
        stable_count = max(1, int(self.force_image_result_stable_count))
        offset_x = float(self.force_image_result_offset_x_m)
        offset_y = float(self.force_image_result_offset_y_m)
        offset_z = float(self.force_image_result_offset_z_m)

        center_px = [float(self.down_release_u), float(self.down_release_v)]
        data = {
            "target": "image_target",
            "detected": True,
            "reason": "forced_result_for_reflection",
            ...
            "release_center_px": center_px,
            "offset_px": [0.0, 0.0],
            "board_area_px": 0.0,
            "board_aspect": 1.0,
            "mapping_method": "forced_zero_offset",
            "classifier_method": "forced_sequence"
        }
        self.publish_result(data)

        # 保留采集功能：即使跳过真实识别，也保存 raw，方便之后复盘/再训练。
        ...
        rospy.logwarn_throttle(
            0.3,
            "[IMAGE_FORCE_RESULT] class=%s conf=%.2f stable=%d offset=(%.3f, %.3f, %.3f)",
    ...
    def process_image_target(self, frame):
        forced_class = self.get_forced_image_result_class()
        if forced_class != "":
            self.publish_forced_image_target(frame, forced_class)
            return
